...
const scheduleToolsClose=()=>{cancelToolsClose();toolsCloseTimer=setTimeout(()=>{if(desktopTools()&&!navTools?.contains(document.activeElement))setToolsMenu(false);},140);};
toolsToggle?.addEventListener('click',event=>{event.stopPropagation();setToolsMenu(!navTools.classList.contains('menu-open'));});
navTools?.addEventListener('pointerenter',()=>{if(desktopTools()){cancelToolsClose();setToolsMenu(true);}});
...
document.addEventListener('click',event=>{if(navTools&&!navTools.contains(event.target))setToolsMenu(false);});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&navTools?.classList.contains('menu-open')){setToolsMenu(false);toolsToggle?.focus();}});
